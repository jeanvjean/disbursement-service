...
const { exportCashout } = require('../../../utils/cashout');
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'exports_cashout_fire':
              {
                console.log(' [Received] %s', message.action);
                await exportCashout(message.data);
                console.log(
                  ' [Processed] %s',
                  `ExportsCashoutWorker - ${message.action}`
                );
          ...
          console.error({ ExportsCashoutWorker: error });
          retry = parseInt(retry + 1);
          if (retry >= 3) {
            // insert job into failed jobs
            await saveFailedJobs({
              worker_name: 'exports_cashout_queue',
