...
const { createAkuPay } = require('../../../utils/applicant');
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'create_aku_account_process':
              {
                console.log(' [Received] %s', message.action);
                await createAkuPay(message.data);
                console.log(
                  ' [Processed] %s',
                  `CreateAkuAccountProcessed - ${message.action}`
                );
          ...
          console.error({ CreateAkuAccountProcessed: error });
          retry = parseInt(retry + 1);
          if (retry >= 3) {
            // insert job into failed jobs
            await saveFailedJobs({
              worker_name: 'create_aku_account_queue',
