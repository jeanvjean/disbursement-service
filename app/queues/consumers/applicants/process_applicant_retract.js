...
const { processRetract } = require('../../../utils/applicant');
...
const run = async () => {
  ...
  const { queue, exchange, routingKey } = await rabbitmqArchitecture(
    'process_applicant_retractment_queue'
  );
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'process_applicant_retractment_process':
              {
                console.log(' [Received] %s', message.action);
                await processRetract(message.data);
                console.log(
                  ' [Processed] %s',
                  `ApplicantRetractProcessed - ${message.action}`
                );
          ...
          if (retry >= 3) {
            ...
            await saveFailedJobs({
              worker_name: 'process_applicant_retractment_queue',
