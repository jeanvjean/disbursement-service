...
const { exportApplicantReply,  sendExportedApplicantReply} = require('../../../utils/applicant_reply');
...
const run = async () => {
  ...
  const { queue, exchange, routingKey } = await rabbitmqArchitecture(
    'exports_applicant_repply_queue'
  );
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'exports_applicant_repply_fire':
              {
                console.log(' [Received] %s', message.action);
                await exportApplicantReply(message.data);
                console.log(
                  ' [Processed] %s',
                  `ExportsapplicantReplysWorker - ${message.action}`
                );
                channel.ack(msg); // acknowledged processing is complete
              }
              break;
              case 'send_exported_applicant_repply_fire':
              {
                console.log(' [Received] %s', message.action);
                await sendExportedApplicantReply(message.data);
                console.log(
                  ' [Processed] %s',
                  `ExportsapplicantReplysWorker - ${message.action}`
                );
          ...
          console.error({ ExportsapplicantReplysWorker: error });
          retry = parseInt(retry + 1);
          if (retry >= 3) {
            // insert job into failed jobs
            await saveFailedJobs({
              worker_name: 'exports_applicant_repply_queue',
