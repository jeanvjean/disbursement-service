...
const { exportSmsLogs } = require('../../../utils/smsLogs');
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'exports_sms_logs_fire':
              {
                console.log(' [Received] %s', message.action);
                await exportSmsLogs(message.data);
                console.log(
                  ' [Processed] %s',
                  `ExportsSmsLogsWorker - ${message.action}`
                );
          ...
          console.error({ ExportsSmsLogsWorker: error });
          retry = parseInt(retry + 1);
          if (retry >= 3) {
            // insert job into failed jobs
            await saveFailedJobs({
              worker_name: 'exports_sms_logs_queue',
