    ...
    async msg => {
      try {
        ...
        switch (`${message.action}_${message.type}`) {
          case 'send_exported_funds_fire':
            {
              console.log(' [Received] %s', message.type);
              await sendExportedFunds(message.data);
              console.log(
                ' [Processed] %s',
                `SendExportsFundsWorker - ${message.type}`
              );
        ...
        console.error({ RetrySendingFundsExportError: error });
        channel.nack(msg);
      }
    },
