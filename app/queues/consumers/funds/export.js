...
const { exportFunds} = require('../../../utils/funds');
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'exports_funds_fire':
              {
                console.log(' [Received] %s', message.action);
                await exportFunds(message.data);
                console.log(
                  ' [Processed] %s',
                  `ExportsFundsWorker - ${message.action}`
                );
          ...
          console.error({ ExportsFundsWorker: error });
          retry = parseInt(retry + 1);
          if (retry >= 3) {
            // insert job into failed jobs
            await saveFailedJobs({
              worker_name: 'exports_funds_queue',
