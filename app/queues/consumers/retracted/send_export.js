    ...
    async msg => {
      try {
        ...
        switch (`${message.action}_${message.type}`) {
          case 'send_exported_retracted_fire':
            {
              console.log(' [Received] %s', message.type);
              await sendExportedRetracted(message.data);
              console.log(
                ' [Processed] %s',
                `SendExportsRetractedWorker - ${message.type}`
              );
        ...
        console.error({ RetrySendingRetractedExportError: error });
        channel.nack(msg);
      }
    },
