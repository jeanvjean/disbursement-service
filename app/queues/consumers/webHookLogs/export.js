...
const { exportWebHookLogs } = require('../../../utils/webHookLogs');
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'exports_webhook_logs_fire':
              {
                console.log(' [Received] %s', message.action);
                await exportWebHookLogs(message.data);
                console.log(
                  ' [Processed] %s',
                  `ExportsWebHookLogsWorker - ${message.action}`
                );
          ...
          console.error({ ExportsWebHookLogsWorker: error });
          retry = parseInt(retry + 1);
          if (retry >= 3) {
            // insert job into failed jobs
            await saveFailedJobs({
              worker_name: 'exports_webhook_logs_queue',
