...
const { sendExportedWebHookLogs } = require('../../../utils/webHookLogs.js');

const run = async () => {
  const { channel } = await rabbitmq();
  const { queue, exchange, routingKey } = await rabbitmqArchitecture(
    'send_exported_webhook_logs_queue'
  );
    ...
    async msg => {
      try {
        ...
        switch (`${message.action}_${message.type}`) {
          case 'send_exported_webhook_logs_fire':
            {
              console.log(' [Received] %s', message.type);
              await sendExportedWebHookLogs(message.data);
              console.log(
                ' [Processed] %s',
                `SendExportsWebHookLogsWorker - ${message.type}`
              );
        ...
        console.error({ RetrySendingWebHookLogsExportError: error });
        channel.nack(msg);
      }
    },
